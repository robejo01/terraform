...
resource "bigip_ltm_node" "appzoneVS-dev-nodes" {
  count   = 2
...
resource "bigip_ltm_virtual_server" "dev-appzones01" {
  name                       = "/${element(var.partitions, 1)}/Dev-AppzoneS01"
  source                     = "0.0.0.0/0"
  mask                       = "255.255.255.255"
  destination                = "${var.az_vip}"
  port                       = 0
  profiles                   = ["/${element(var.partitions, 0)}/tcp"]
  client_profiles            = ["/${element(var.partitions, 1)}/DEV-AppzoneS01"]
  pool                       = "/${element(var.partitions, 1)}/Dev-AppzoneS01-Pool"
...
resource "bigip_ltm_pool" "az_pool" {
  name                = "/${element(var.partitions, 1)}/Dev-AppzoneS01-Pool"
...
# Victoria Station Pool Attachment
