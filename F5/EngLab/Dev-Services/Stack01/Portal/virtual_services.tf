# UDX Nodes
resource "bigip_ltm_node" "portals01-dev-nodes" {
  count   = 2
...
resource "bigip_ltm_virtual_server" "dev-portals01" {
  name                       = "/${element(var.partitions, 1)}/Dev-PortalS01"
  source                     = "0.0.0.0/0"
  mask                       = "255.255.255.255"
  destination                = "${var.udx_vip}"
  port                       = 443
  profiles                   = ["/${element(var.partitions, 0)}/tcp"]
  client_profiles            = ["/${element(var.partitions, 1)}/DEV-PortalS01"]
  pool                       = "/${element(var.partitions, 1)}/Dev-PortalS01-Pool"
...
resource "bigip_ltm_pool" "udx_pool" {
  name                = "/${element(var.partitions, 1)}/Dev-PortalS01-Pool"
