...
resource "bigip_ltm_virtual_server" "dev-appzones02" {
  name                       = "/${element(var.partitions, 1)}/Dev-AppzoneS02"
  source                     = "0.0.0.0/0"
  mask                       = "255.255.255.255"
  destination                = "${var.az_vip}"
  port                       = 0
  profiles                   = ["/${element(var.partitions, 0)}/tcp"]
  client_profiles            = ["/${element(var.partitions, 1)}/DEV-AppzoneS02"]
  pool                       = "/${element(var.partitions, 1)}/Dev-AppzoneS02-Pool"
...
resource "bigip_ltm_pool" "az_pool" {
  name                = "/${element(var.partitions, 1)}/Dev-AppzoneS02-Pool"
