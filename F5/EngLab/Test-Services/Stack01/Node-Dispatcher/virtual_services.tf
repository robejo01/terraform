# BDispatcher Nodes
resource "bigip_ltm_node" "bdispatchs01-test-nodes" {
  count   = 2
...
resource "bigip_ltm_virtual_server" "test-bdispatchs01" {
  name                       = "/${element(var.partitions, 1)}/Test-BDispatchs01"
  source                     = "0.0.0.0/0"
  mask                       = "255.255.255.255"
  destination                = "${var.bdisp_vip}"
  port                       = 38040
  profiles                   = ["/${element(var.partitions, 0)}/http"]
  pool                       = "/${element(var.partitions, 1)}/Test-BDispatchs01-Pool"
...
resource "bigip_ltm_pool" "bdisp_pool" {
  name                = "/${element(var.partitions, 1)}/Test-BDispatchs01-Pool"
