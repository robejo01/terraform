# BDScheduler Nodes
resource "bigip_ltm_node" "bschedules01-test-nodes" {
  count   = 2
...
resource "bigip_ltm_virtual_server" "test-bschedules01" {
  name                       = "/${element(var.partitions, 1)}/Test-BSchedules01"
  source                     = "0.0.0.0/0"
  mask                       = "255.255.255.255"
  destination                = "${var.bsch_vip}"
  port                       = 38050
  profiles                   = ["/${element(var.partitions, 0)}/http"]
  pool                       = "/${element(var.partitions, 1)}/Test-BSchedules01-Pool"
...
resource "bigip_ltm_pool" "bsch_pool" {
  name                = "/${element(var.partitions, 1)}/Test-BSchedules01-Pool"
