# UDX Nodes
resource "bigip_ltm_node" "portals01-test-nodes" {
  count   = 2
...
resource "bigip_ltm_virtual_server" "test-portals01" {
  name                       = "/${element(var.partitions, 1)}/Test-PortalS01"
  source                     = "0.0.0.0/0"
  mask                       = "255.255.255.255"
  destination                = "${var.udx_vip}"
  port                       = 443
  profiles                   = ["/${element(var.partitions, 1)}/laravel_portal_http"]
  client_profiles            = ["/${element(var.partitions, 1)}/TEST-PortalS01"]
  pool                       = "/${element(var.partitions, 1)}/Test-PortalS01-Pool"
...
resource "bigip_ltm_pool" "udx_pool" {
  name                = "/${element(var.partitions, 1)}/Test-PortalS01-Pool"
