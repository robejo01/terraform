...
resource "null_resource" "appzone_bkend1" {
  ...
  provisioner "local-exec" {
    command = <<EOT
    curl -s -i -k -X POST -H 'Content-Type: application/json' -H "ZAPI_KEY: "${var.api_key}"" -d '{"ip":"${var.az_bkend1_ip}","port":${var.az_bkend_port},"weight":2,"timeout":5}' https://zenlb01.mgmt.tsafe.systems:444/zapi/v4.0/zapi.cgi/farms/dev-appzones01/services/appzones01-svc/backends
EOT
  }
}
...
resource "null_resource" "appzone_bkend2" {
  ...
  provisioner "local-exec" {
    command = <<EOT
    curl -s -i -k -X POST -H 'Content-Type: application/json' -H "ZAPI_KEY: "${var.api_key}"" -d '{"ip":"${var.az_bkend2_ip}","port":${var.az_bkend_port},"weight":2,"timeout":5}' https://zenlb01.mgmt.tsafe.systems:444/zapi/v4.0/zapi.cgi/farms/dev-appzones01/services/appzones01-svc/backends
EOT
  }
}
