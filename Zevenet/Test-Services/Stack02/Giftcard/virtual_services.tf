...
resource "null_resource" "gc-vinterface" {
  provisioner "local-exec" {
    command = <<EOT
  curl -s -i -k -X POST -H 'Content-Type: application/json' -H "ZAPI_KEY: "${var.api_key}"" -d '{"ip":"${var.gc_vint_ip}","name":"${var.gc_vint_name}"}' https://zenlb03.mgmt.tsafe.systems:444/zapi/v4.0/zapi.cgi/interfaces/virtual
EOT
  }
}
...
resource "null_resource" "gc-farm" {
  ...
  provisioner "local-exec" {
    command = <<EOT
  curl -s -i -k -X POST -H 'Content-Type: application/json' -H "ZAPI_KEY: "${var.api_key}"" -d '{"farmname":"test-giftcardsvcs02","profile":"http","vip":"${var.gc_vint_ip}","vport":"13010"}' https://zenlb03.mgmt.tsafe.systems:444/zapi/v4.0/zapi.cgi/farms
EOT
  }
}
...
resource "null_resource" "farmsvc_restart-2" {
  ...
  provisioner "local-exec" {
    command = <<EOT
    curl -s -i -k -X PUT -H 'Content-Type: application/json' -H "ZAPI_KEY: "${var.api_key}"" -d '{"action":"restart"}' https://zenlb03.mgmt.tsafe.systems:444/zapi/v4.0/zapi.cgi/farms/test-giftcardsvcs02/actions
EOT
  }
}
...
resource "null_resource" "gc_service" {
  ...
  provisioner "local-exec" {
    command = <<EOT
    curl -s -i -k -X POST -H 'Content-Type: application/json' -H "ZAPI_KEY: "${var.api_key}"" -d '{"id":"giftcardsvcs02-svc"}' https://zenlb03.mgmt.tsafe.systems:444/zapi/v4.0/zapi.cgi/farms/test-giftcardsvcs02/services
EOT
  }
}
...
resource "null_resource" "gc_svcmod" {
  ...
  provisioner "local-exec" {
    command = <<EOT
    curl -s -i -k -X PUT -H 'Content-Type: application/json' -H "ZAPI_KEY: "${var.api_key}"" -d '{"vhost":"${var.gc_vhostname}","leastresp":"true"}' https://zenlb03.mgmt.tsafe.systems:444/zapi/v4.0/zapi.cgi/farms/test-giftcardsvcs02/services/giftcardsvcs02-svc
EOT
  }
}
...
resource "null_resource" "gc_bkend1" {
  ...
  provisioner "local-exec" {
    command = <<EOT
    curl -s -i -k -X POST -H 'Content-Type: application/json' -H "ZAPI_KEY: "${var.api_key}"" -d '{"ip":"${var.gc_bkend1_ip}","port":${var.gc_bkend_port},"weight":2,"timeout":15}' https://zenlb03.mgmt.tsafe.systems:444/zapi/v4.0/zapi.cgi/farms/test-giftcardsvcs02/services/giftcardsvcs02-svc/backends
EOT
  }
}
...
resource "null_resource" "gc_bkend2" {
  ...
  provisioner "local-exec" {
    command = <<EOT
    curl -s -i -k -X POST -H 'Content-Type: application/json' -H "ZAPI_KEY: "${var.api_key}"" -d '{"ip":"${var.gc_bkend2_ip}","port":${var.gc_bkend_port},"weight":2,"timeout":15}' https://zenlb03.mgmt.tsafe.systems:444/zapi/v4.0/zapi.cgi/farms/test-giftcardsvcs02/services/giftcardsvcs02-svc/backends
EOT
  }
}
...
resource "null_resource" "add-gc-check" {
  ...
  provisioner "local-exec" {
    command = <<EOT
    curl -s -i -k -X POST -H 'Content-Type: application/json' -H "ZAPI_KEY: "${var.api_key}"" -d '{"name":"check_giftcard","copy_from":"check_http"}' https://zenlb03.mgmt.tsafe.systems:444/zapi/v4.0/zapi.cgi/monitoring/fg
EOT
  }
}
...
resource "null_resource" "mod-gc-check" {
  ...
  provisioner "local-exec" {
    command = <<EOT
    curl -s -i -k -X PUT -H 'Content-Type: application/json' -H "ZAPI_KEY: "${var.api_key}"" -d '{"command":"check_http -I HOST -p PORT -w 10 -c 15 -t 15 -u / -R \\\"health\\\":\\\"HEALTHY\\\""}' https://zenlb03.mgmt.tsafe.systems:444/zapi/v4.0/zapi.cgi/monitoring/fg/check_giftcard
EOT
  }
}
...
resource "null_resource" "gc-check" {
  ...
  provisioner "local-exec" {
    command = <<EOT
    curl -s -i -k -X POST -H 'Content-Type: application/json' -H "ZAPI_KEY: "${var.api_key}"" -d '{"name":"check_giftcard"}' https://zenlb03.mgmt.tsafe.systems:444/zapi/v4.0/zapi.cgi/farms/test-giftcardsvcs02/services/giftcardsvcs02-svc/fg
EOT
  }
}
