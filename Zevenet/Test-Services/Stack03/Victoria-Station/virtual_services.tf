...
resource "null_resource" "vs-vinterface" {
  provisioner "local-exec" {
    command = <<EOT
  curl -s -i -k -X POST -H 'Content-Type: application/json' -H "ZAPI_KEY: "${var.api_key}"" -d '{"ip":"${var.vs_vint_ip}","name":"${var.vs_vint_name}"}' https://zenlb03.mgmt.tsafe.systems:444/zapi/v4.0/zapi.cgi/interfaces/virtual
EOT
  }
}
...
resource "null_resource" "vs-farm" {
  ...
  provisioner "local-exec" {
    command = <<EOT
  curl -s -i -k -X POST -H 'Content-Type: application/json' -H "ZAPI_KEY: "${var.api_key}"" -d '{"farmname":"test-vssvcs03","profile":"http","vip":"${var.vs_vint_ip}","vport":"13010"}' https://zenlb03.mgmt.tsafe.systems:444/zapi/v4.0/zapi.cgi/farms
EOT
  }
}
...
resource "null_resource" "farmsvc_restart-1" {
  ...
  provisioner "local-exec" {
    command = <<EOT
    curl -s -i -k -X PUT -H 'Content-Type: application/json' -H "ZAPI_KEY: "${var.api_key}"" -d '{"action":"restart"}' https://zenlb03.mgmt.tsafe.systems:444/zapi/v4.0/zapi.cgi/farms/test-vssvcs03/actions
EOT
  }
}
...
resource "null_resource" "vs_service" {
  ...
  provisioner "local-exec" {
    command = <<EOT
    curl -s -i -k -X POST -H 'Content-Type: application/json' -H "ZAPI_KEY: "${var.api_key}"" -d '{"id":"vssvcs03-svc"}' https://zenlb03.mgmt.tsafe.systems:444/zapi/v4.0/zapi.cgi/farms/test-vssvcs03/services
EOT
  }
}
...
resource "null_resource" "vs_svcmod" {
  ...
  provisioner "local-exec" {
    command = <<EOT
    curl -s -i -k -X PUT -H 'Content-Type: application/json' -H "ZAPI_KEY: "${var.api_key}"" -d '{"vhost":"${var.vs_vhostname}"}' https://zenlb03.mgmt.tsafe.systems:444/zapi/v4.0/zapi.cgi/farms/test-vssvcs03/services/vssvcs03-svc
EOT
  }
}
...
resource "null_resource" "vs_bkend1" {
  ...
  provisioner "local-exec" {
    command = <<EOT
    curl -s -i -k -X POST -H 'Content-Type: application/json' -H "ZAPI_KEY: "${var.api_key}"" -d '{"ip":"${var.vs_bkend1_ip}","port":${var.vs_bkend_port},"weight":2,"timeout":15}' https://zenlb03.mgmt.tsafe.systems:444/zapi/v4.0/zapi.cgi/farms/test-vssvcs03/services/vssvcs03-svc/backends
EOT
  }
}
...
resource "null_resource" "vs_bkend2" {
  ...
  provisioner "local-exec" {
    command = <<EOT
    curl -s -i -k -X POST -H 'Content-Type: application/json' -H "ZAPI_KEY: "${var.api_key}"" -d '{"ip":"${var.vs_bkend2_ip}","port":${var.vs_bkend_port},"weight":2,"timeout":15}' https://zenlb03.mgmt.tsafe.systems:444/zapi/v4.0/zapi.cgi/farms/test-vssvcs03/services/vssvcs03-svc/backends
EOT
  }
}
...
resource "null_resource" "add-vstation-check" {
  ...
  provisioner "local-exec" {
    command = <<EOT
    curl -s -i -k -X POST -H 'Content-Type: application/json' -H "ZAPI_KEY: "${var.api_key}"" -d '{"name":"check_vstation","copy_from":"check_http"}' https://zenlb03.mgmt.tsafe.systems:444/zapi/v4.0/zapi.cgi/monitoring/fg
EOT
  }
}
...
resource "null_resource" "mod-vstation-check" {
  ...
  provisioner "local-exec" {
    command = <<EOT
    curl -s -i -k -X PUT -H 'Content-Type: application/json' -H "ZAPI_KEY: "${var.api_key}"" -d '{"command":"check_http -I HOST -p PORT -w 10 -c 15 -t 15 -u / -R \\\"health\\\":\\\"HEALTHY\\\""}' https://zenlb03.mgmt.tsafe.systems:444/zapi/v4.0/zapi.cgi/monitoring/fg/check_vstation
EOT
  }
}
...
resource "null_resource" "vstation-check" {
  ...
  provisioner "local-exec" {
    command = <<EOT
    curl -s -i -k -X POST -H 'Content-Type: application/json' -H "ZAPI_KEY: "${var.api_key}"" -d '{"name":"check_vstation"}' https://zenlb03.mgmt.tsafe.systems:444/zapi/v4.0/zapi.cgi/farms/test-vssvcs03/services/vssvcs03-svc/fg
EOT
  }
}
